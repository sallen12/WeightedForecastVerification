...
%\VignetteIndexEntry{Weighted Forecast Verification}
%\VignetteDepends{WeightedForecastVerification}
%\VignetteKeywords{probabilistic forecasts, scoring rules, weighted scoring rules, calibration}
%\VignettePackage{WeightedForecastVerification}
...
\title{\pkg{WeightedForecastVerification}: Weighted Verification Tools to Evaluate Probabilistic Forecasts}
\Plaintitle{WeightedForecastVerification: Weighted Verification Tools to Evaluate Probabilistic Forecasts}
\Shorttitle{WeightedForecastVerification}

\Abstract{
  Probabilistic forecasts are often assessed and compared using proper scoring rules. To emphasise particular outcomes when evaluating probabilistic forecasts, various approaches have been proposed to construct weighted scoring rules, both in the univariate and multivariate setting. \cite{AllenEtAl2023} discuss and compare these different approaches, and illustrate how the theory underlying weighted scoring rules can also be applied when assessing forecast calibration. This vignette reproduces the results presented in this paper, and demonstrates how the accompanying \proglang{R} package allows these weighted verification tools to be implemented in practice.
}
...
Methods to verify and evaluate forecasts are integral to the development and improvement of forecast systems. Traditionally, the evaluation of probabilistic forecasts focuses on two aspects of forecast performance: forecast accuracy and forecast calibration. Forecast accuracy is a measure of the agreement between a forecast and the corresponding observation, and can be quantified using proper scoring rules. Scoring rules summarise forecast performance using a single numerical value, allowing competing forecasters to be ranked and compared objectively; \emph{proper} scoring rules further encourage the forecaster to issue what they truly believe will occur \citep{GneitingRaftery2007}. Forecast calibration, on the other hand, considers to what extent forecasts are reliable, or trustworthy - for example, do the observed outcomes occur with the same probability with which they are predicted? This is typically assessed visually using graphical diagnostic tools, though statistical tests also exist to check calibration more rigorously.

However, it is often the case that certain outcomes are of more interest than others: when predicting events that can have a high impact on the forecast user, for example. Hence, over the past decade or so, several approaches have been developed to target particular outcomes during forecast evaluation. This is typically achieved by incorporating a weight function into conventional methods. \cite{AllenEtAl2023} recently reviewed weighted verification tools, and the goal of this vignette is to demonstrate how the accompanying \proglang{R} package allows users to implement these tools in practical applications. The package has not been submitted to CRAN, and is still in active development; some potential extensions to the existing functionality are discussed in the final section.
...
\end{equation}
where $y = (y_{1}, \dots, y_{d}) \in \mathbbm{R}^{d}$, $X = (X_{1}, \dots, X_{d}) \sim F$, $h_{i, j}$ are non-negative scaling parameters, and $p > 0$ is the order of the score. Typically, $ p $ is chosen to be one half, and the scaling parameters $h_{i, j}$ are either set to one, or, if there is some notion of distance between different dimensions, they are inversely proportional to this distance.
...
Weighted scoring rules can be implemented analogously for Student's $t$ and logistic distributions,

\begin{Code}
twcrps_logis(y, location = 0, scale = 1, a = -Inf, b = Inf)
owcrps_logis(y, location = 0, scale = 1, a = -Inf, b = Inf, BS = T)
twcrps_t(y, df, ncp = 0, a = -Inf, b = Inf)
owcrps_t(y, df, ncp = 0, a = -Inf, b = Inf, BS = T)
\end{Code}
where \code{location} and \code{scale} are location and scale parameters of the logistic distribution, and \code{df} and \code{ncp} are the degrees of freedom and non-centrality parameters of the Student's $t$ distribution.

The weighted versions of the CRPS are similarly displayed for a standard logistic forecast distribution in Figure \ref{fig:scores_logis_example}.
...
\end{Code}
Here, \code{dat} is a matrix with each column containing a sample or ensemble member. To calculate the conditional PIT values, the sample is first smoothed using kernel density estimation; note that this can lead to unreliable results if interest is on outcomes that exceed all sample members, since the conditional PIT values will rely on how the density estimation extrapolates beyond the observed sample. The \code{bw} argument specifies the bandwidth parameter to use within kernel density estimation; if this is not provided, then it is selected automatically based on the data.
...
# get mean scores
wcrps_clim <- colMeans(wcrps_clim)
wcrps_raw <- colMeans(wcrps_raw, na.rm=T) # note: generally should not ignore cases where the score is not defined - done so here for illustration
wcrps_pp <- colMeans(wcrps_pp)
...
Finally, \cite{AllenEtAl2023} additionally discuss how conditional multivariate rank and PIT histograms could be constructed. There is currently no software that contains the functionality to implement checks for multivariate calibration (though a developmental package is available at \url{https://github.com/sallen12/MultivCalibration}), and this could relatively easily be incorporated into this package, with the ultimate goal of providing a complete collection of methods to assess probabilistic forecast calibration. This could include, for example: reliability diagrams, rank histograms, (conditional) PIT histograms, (conditional) PIT reliability diagrams, (conditional) multivariate rank histograms, reliability indices, tests for calibration, sequential measures of calibration, functional-calibration reliability diagrams, and proper score decompositions, among other things.


%% Bibliography
\bibliography{bibliography}

\section*{Appendix: Analytical formulae for weighted scores}

\subsection*{Threshold-weighted CRPS}

Consider the weight function $w(z) = \ind\{a < z < b\}$. The threshold-weighted CRPS with this weight function is
\[
\int_{a}^{b} (F(z) - \ind \{ y \leq z\})^{2} \dd z,
\]
which is equivalent to the CRPS for a distribution that has been censored below at $a$ and censored above at $b$ (with $y$ also censored above and below at $a$ and $b$).

\citet[Appendix A.4]{JordanEtAl2019} provide analytical formulae for this censored distribution when the forecast $F$ is a normal, logistic, or Student's $t$ distribution. The functions \code{twcrps_norm()}, \code{twcrps_logis()}, and \code{twcrps_t()} therefore contain calls to the \pkg{scoringRules} functions \code{crps_cnorm()}, \code{crps_clogis()}, and \code{crps_ct()}.


\subsection*{Outcome-weighted CRPS}

Consider the weight function $w(z) = \ind\{a < z < b\}$. The outcome-weighted CRPS with this weight function is
\[
\ind\{a < y < b\} \int_{-\infty}^{\infty} \left( \frac{F(z) - F(a)}{F(b) - F(a)} - \ind \{ y \leq z\} \right)^{2} \dd z,
\]
which is equal to zero if $y$ is not between $a$ and $b$, and is equivalent to the CRPS for a distribution that has been truncated below at $a$ and above at $b$, otherwise.

\citet[Appendix A.4]{JordanEtAl2019} provide analytical formulae for this truncated distribution when the forecast $F$ is a normal, logistic, or Student's $t$ distribution. The functions \code{owcrps_norm()}, \code{owcrps_logis()}, and \code{owcrps_t()} therefore contain calls to the \pkg{scoringRules} functions \code{crps_tnorm()}, \code{crps_tlogis()}, and \code{crps_tt()}.


\subsection*{Vertically re-scaled CRPS}

Suppose the forecast is a normal distribution with mean $\mu$ and standard deviation $\sigma$, and that we observe the value $y$. Consider the weight function $w(z) = \ind\{a < z < b\}$, and let $x_{0} \in \R$. Additionally, let $a^{+} = \mathrm{max}(y, a)$, $a^{-} = \mathrm{min}(y, a)$, $b^{+} = \mathrm{max}(y, b)$, $b^{-} = \mathrm{min}(y, b)$, and $a_{0}^{+} = \mathrm{max}(x_{0}, a)$, $a_{0}^{-} = \mathrm{min}(x_{0}, a)$, $b_{0}^{+} = \mathrm{max}(x_{0}, b)$, $b_{0}^{-} = \mathrm{min}(x_{0}, b)$.

Then, the vertically re-scaled CRPS for this forecast distribution can be written analytically as

\begin{equation*}
\begin{split}
  \mathrm{vrCRPS}(F, y) =& w(y) \left\{ (y - \mu) \left[ F(b^{-}) + F(a^{+}) - F(b^{+}) - F(a^{-}) \right] \right. \\
  & + \left. \sigma^{2} \left[ f(b^{-}) + f(a^{+}) - f(b^{+}) - f(a^{-}) \right] \right\} \\
  & - \frac{\sigma}{\sqrt{\pi}} \left[ \Phi \left( \sqrt{2} \frac{b - \mu}{\sigma} \right) - \Phi \left( \sqrt{2} \frac{a - \mu}{\sigma} \right) \right] + \sigma^{2} \left[ f(b) + f(a) \right] \left[ F(b) - F(a) \right] \\
  & + \left\{ (x_{0} - \mu) \left[ F(b_{0}^{-}) + F(a_{0}^{+}) - F(b_{0}^{+}) - F(a_{0}^{-}) \right] + \right. \\
  & \left. \sigma^{2} \left[ f(b_{0}^{-}) + f(a_{0}^{+}) - f(b_{0}^{+}) - f(a_{0}^{-}) \right] - |y - x_{0}|w(y) \right\} \left\{ F(b) - F(a) - w(y) \right\}.
\end{split}
\end{equation*}
